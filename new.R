...
# Sexto archivo
  ...
  rownames <- getSheetNames("Data/Municipios8.xlsx")
  
  rownames <- gsub(" ", "", rownames)
  
  i <- 1
  municipio <- read_excel("Data/Municipios8.xlsx", 
                          sheet = i, range = "B3:D19", col_names = c("Medicina Legal", "Policía", "Fiscalía")) 
  ...
  settasas7 <- as_tibble(cbind(settasas, matrix(0, ncol=17, nrow=length(rownames)), setconteos, setfuentes))
  
  for(k in 5:21){
    for(i in t(settasas7[,1])){
      settasas7[settasas7[,1]==i,k] <- (as.numeric(settasas7[settasas7[,1]==i, k + 17])/Poblacion[Poblacion$DPMP == i,k+2])*100000
    }
    colnames(settasas7)[k] <- paste0(2000+k-3,".tasa")
...
colnames(propordef)[25:41] <- colnames(settasasall[25:41])
